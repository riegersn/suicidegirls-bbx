...
def check_authentication():
	...
	sg.SetUserAgent('Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.7) Gecko/2007091417 Firefox/2.0.0.7')
...
def login_sg(user, password):
	...
	mc.ShowDialogNotification("Authenticating to SuicideGirls...", "sg_icon_notification.png")
	mc.ShowDialogWait()
	config = mc.GetApp().GetLocalConfig()
	...
	if 'PHPSESSID=' in responseCookie:
		...
		mc.ShowDialogNotification("Welcome to SuicideGirls!", "sg_icon_notification.png")
		mc.LogDebug('SuicideGirls: Authentication successfull')
		return True
	#elif 'LOGIN_ERROR=' in responseCookie:
	#	login_error = responseCookie.split(';')
	#	for val in login_error:
	#		if val.strip().startswith('LOGIN_ERROR'):
	#			val = val.split('=')
	#			login_error = val[1]
	#			login_error = urllib.unquote_plus(login_error.strip())
	#			mc.ShowDialogNotification("Authentication Error: ("+login_error+") Please check your email!", "sg_icon_notification.png")
	#	mc.LogDebug('SuicideGirls: Authentication failed: '+login_error)
	else:
		mc.ShowDialogNotification("Authentication failed!", "sg_icon_notification.png")
...
def search_sg(string):
	...
	try:
		...
		for girl in results:
			...
			item.SetLabel(girl)
			item.SetPath("rss://dir.boxee.tv/apps/suicidegirls/girls/model=%s" % (urllib.quote(girl)))
			item.SetThumbnail("http://img.suicidegirls.com/media/girls/%s/girl_pic_medium.jpg" % (urllib.quote(girl)))
		...
		mc.ShowDialogNotification("An error has occurred while searching!", "sg_icon_notification.png")
